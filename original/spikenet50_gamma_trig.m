function spikenet50_gamma_trig(folder)
% Runs an integerate and fire neural network model.
% Displays correlograms and spike triggered averages for selected units.
%
% This version has been setup for gamma triggered conditioning. This is a
% modification of spikenet50.m with:
%   p.conditioning_type = 8;
%   p.stim_delay_ms = 0;          % Millisecond delay between gamma level detection and stimulus on Column B. (use value 8+ for rising level detection)
%   p.lfp_detect_level = -20000;  % Filtered LFPA detection level (20000 for rising level, -20000 for falling). This may need to be modified based on overall activity levels.
...
p.conditioning_type = 8;       % 0=No conditioning, 1=spike triggered, 2=paired pulse, 3 = cycle triggered on LFPB, 4 = Tetanic stim on Col A, 5 = Tetanic stim on Col B, 6 = Tetatic stim on Col C, 7 = EMG triggered Stimulation, 8 = Gamma triggered stimulation, 9 = Activity triggered stimulation
p.stim_delay_ms = 0;           % Stimulaton delay from rec_unit threshold crossing to start of stimulation PSP in designated stim target units.
...
p.lfp_detect_level = -20000;    % Amplitude of LFP used for cycle triggered conditioning (use 30000 for cycle triggered +/-20%, -20000 or so for gamma triggered depending on %correlated bias drive, 1000 for EMG triggered
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    function Init_Weight(pre_name, pre_index, post_name, post_index, initial_psp)
        ...
        save_deltat_hist = 0;
        if (pre_name(1) == 'A') && (post_name(1) ~= 'A')
            save_deltat_hist = 1; % Save STDP delta t histograms for weights A->B and A->C
        end
